...
class MemoSet(BaseModel):
    ...
    owner       = models.ForeignKey(User,
                help_text=_('Memo Set is owned by this user'), null=True, blank=True,
                related_name='memosets', on_delete=models.CASCADE)
    parent      = models.ForeignKey('MemoSet',
                help_text=_('Parent Memo Set'), null=True, blank=True,
                related_name='children', on_delete=models.CASCADE)
    topic       = models.CharField(_("Topic"),
                help_text=_('Topic of Memo Card'), max_length=255)
    is_template = models.BooleanField(_('Template'),
                help_text=_('This set can be forked by other users'), default=False)
    # TODO: pause-toogle
...
class MemoCard(BaseModel):
    ...
    memoset         = models.ForeignKey(MemoSet,
                    help_text=_('Memo Card is assigned to this set'),
                    related_name='memocards', on_delete=models.CASCADE))
    topic           = models.CharField(_("Topic"), help_text=_('Topic of Memo Card'), max_length=255)
    ...
    data            = models.JSONField(_('Memorization Data'), encoder=DjangoJSONEncoder)
    score           = models.DecimalField(_("Memorization Score"),
                    help_text=_('Memo Card is assigned to this set'),
